...
ggplot(data=epa_pm25_sites_sf) +
  geom_sf()
# sf = spatial feature or spatial dataframe
# spatial features need: coordinate system, geometries, and attributes
```
...
#Create a map
mapview(huc8_sf, 
        col.regions = "orange", layer.name = "HUC 8") + 
  mapView(upperneuse_sf, col.regions = "purple", layer.name = "Upper Neuse") 
...
# use tidyverse to join HUC_8 features
# another example of using tidyverse
...
neuse_intersect_2 <- counties_sf_utm %>% 
  # x = ., is a filler for the counties_sf_utm
  filter(st_intersects(x = ., y = neuse_sf, sparse = FALSE))
# can also use st_filter(dataframe) to just do a basic intersection
...
the_triangle <- counties_sf_utm %>%
  filter(NAME %in% c("Chathan", "Durham", "Orange", "Wake"))
...
#Right order, check what will cover what
ggplot() +
  geom_sf(data = state_sf_utm, color='red',size=2) +
  geom_sf(data = counties_sf_utm, aes(fill = ALAND), color = 'white')  +
...
# Convert all to WGS84 (crs=4326), be/c this is the leaflet base layer
