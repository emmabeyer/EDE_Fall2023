...
# !!! ask TA to look at my path to make sure it's ok. Not sure what guest means.
```
...
#Can use "tab" to select relative path. Put in read.csv("./) and then press tab, options should appear. Press tab again to see next menu and so on
...
str(USGS.flow.data) #shows the data frame and all the objects in the data frame
dim(USGS.flow.data) #shows how many row and then how many columns
length(USGS.flow.data) #shows how many columns

head(USGS.flow.data) #shows the first few rows of the data. Auto generates first 6 rows
head(USGS.flow.data, 10) #shows the first 10 rows
tail(USGS.flow.data, 5) #shows the last 5 rows of the data set
USGS.flow.data[30000:30005, c(3, 8, 14)] #shows rows 30000 through 30005 and only columns 3, 8, and 14

class(USGS.flow.data$datetime) #shows the class of the data, numeric or character. This example will show row "flow.data" and column "datetime"
class(USGS.flow.data$discharge.mean)
class(USGS.flow.data$gage.height.mean)

summary(USGS.flow.data)  #shows summary stats of the whole data set. Could point to column only by adding $

```

What happened to blank cells in the spreadsheet when they were imported into R?
> Answer: Replaced with NA
...
```{r Removing NAs}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #creating new data set without NAs. Always keep the raw data unedited, and make a new data set
dim(USGS.flow.data)
dim(USGS.flow.data.complete)

mean(USGS.flow.data.complete$discharge.mean) #now can calc stats w/out NAs
...
%y  2-digit year ex. 99
%Y  4-digit year ex. 1999
%a  abbreviated weekday  ex. Mon
%A  unabbreviated weekday  ex. Monday
%b  abbreviated month  ex. Jan
%B  unabbreviated month  ex. January

note - % is used to format a variable
...
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y")  #changing the date format. First is date column, second is format
```
...
```{r Fixing dates}
# changed data into a simple format
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d")

#connects strings together. Assigning 19 to dates after 1968 and assigning 20 above that
create.early.dates <- (function(d) {
       paste0(ifelse(d > 191226,"19","20"),d)
       })
#making nre adjusted data set
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)

#transforming date back to year, month, day
...
Avoid install.package() and View () and commands that produce long outputs. 
Restart your R session and run code from start to finish. Ensure all packages are installed, objects are created in code itself.
Check WD and relative paths.
Tidy R code w/ breaks so they don't extend off the page.
Make sure cluncks don't have any symbols or characters. Just stick to works
