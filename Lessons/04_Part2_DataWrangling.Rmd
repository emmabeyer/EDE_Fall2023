...
# Format sampledate as date, always need to do this when I export data
...
NTL.nutrient.data.PeterPaul <- 
  ...
  drop_na(depth)
#pipeline filtering of peterpaul
...
#gather nutrient data into two columns and add that to your data set

NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))
#removes the NA rows in the data set
count(NTL.nutrient.data.PeterPaul.gathered$nutrient)
#didn't workk with comma, so I used $, ask why that is
...
# I recommend using lubridate rather than separate and unite. See example in pt.1
...
#import all the data individually and then combine them, but not as efficient
```
...
LitterFiles
#finds all the files that we want to read

Litter <- LitterFiles %>%
  plyr::ldply(read.csv)
#use pipeline to read all of these files
```
...
#select is only treating columns
