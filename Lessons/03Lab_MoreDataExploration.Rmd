...
vector1[3] # pointing towards element in the vector

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # matrix with 5 rows and 4 columns
matrix1
...
matrix1[4, ] # before comma is row, saying I want the whole 4th row
matrix1[ , 3] # saying I want whole 3rd column
matrix1[c(12, 14)] # finds specific values
matrix1[c(12:14)] # finds sequence of values
matrix1[2:4, 1:3] # finds specific sections of values
...
f <- c(TRUE, TRUE, TRUE, FALSE) # What type of vector? Factors
...
```{r DataExercise-1a}
ten_day_forcast_high_fahrenheit <- c(79, 81, 81, 82, 76, 68, 76, 80)
ten_day_forcast_low_fahrenheit <- c(53, 54, 56, 62, 63, 61, 64, 64)
...
```{r DataExercise-1b}
ten_day_forcast_high_celsius <- (ten_day_forcast_high_fahrenheit-32)*5/9
ten_day_forcast_low_celsius <- (ten_day_forcast_low_fahrenheit-32)*5/9
...
```{r DataExercise-1c}
df_temp <- data.frame("HighTempF"=ten_day_forcast_high_fahrenheit, ten_day_forcast_low_fahrenheit, ten_day_forcast_low_celsius, ten_day_forcast_high_celsius)
...
```{r DataExercise-1d}
summary(df_temp)
sd(df_temp$HighTempF) #need to make the data frame a specific column
