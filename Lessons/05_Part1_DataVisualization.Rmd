...
USE ABOVE LINK TO DECIDE WHAT PLOT TO USE FOR MY DATA
...
* geom_line
* geom_area
* geom_abline (plus geom_hline (horizonal line) and geom_vline(vertical line))
* geom_text
...
# above the fig.height etc is setting the bounds for our plots. This will apply to all the plot be/c its in the top clunk code
# Scatterplot
...
print(O3plot)
#same plots, but want us to create object instead of above 
...
O3plot2 <- ggplot(EPAair) +
  ...
  geom_point(aes(x = Date, y = Ozone), color = "blue"))
print(O3plot2)
#need to have the color function outside of the aesthetics or else it won't work
...
PMplot <- 
  ...
  geom_point()
print(PMplot)
# 'shape = as.factor(Year)' automatically assigns a shape to each year
# 'color = Site.Name' will automatically assign a different color to each site name and create a legend (since it was included in aes)
...
#replacing 'color =' with geom_point and facet_wrap
#separates each site, but lose color
...
#selecting the sites you want using the subset function and separating the years with facet_grid function
...
PMplot.line <- 
  ...
  geom_line()
print(PMplot.line)
#example of a time series plot (will see this more in later lesson)
...
lightvsDO <- 
  ...
  geom_point()
print(lightvsDO)
#not a good representation of the data be/c of outliers
...
# excludes outliers and zooms in on most relevant data
...
tempvsdepth <- 
  ...
  scale_y_reverse()
print(tempvsdepth)
#'scale_y_reverse' puts the 0 at the top of the graph instead of the bottom. This is good for depth graphs
...
print(NvsP)
# 'geom_smooth' adds trend line be/w two nutrients
# 'geom_abline' adding our own line with a slope of 16
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
# These are better to display summary stats than bar plot w/ error bars
# Box and whiskers plot
Nutrientplot3 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? Fill will keep the contore dots black, while color adds the color to the dots as well as box plot
print(Nutrientplot3)
...
print(Nutrientplot4)
# now can see the average values better than box plot
# 'biaxis = "y"' means stack bins over y axis
# stackdir stacks the dots in the center
# dodge stacks values with the same value next to each other (wider in graph)
...
print(Nutrientplot5)
# hard to see density of data at bottom of graph
...
print(Nutrientplot6)
# specifying x axis and color
# doesn't have a legend or specify be/w nutrients
...
print(Nutrientplot7)
# now w/ gathering data, much easier to plot data

# Frequency polygons have the risk of becoming spaghetti plots. Or having too many messy lines
...
print(Nutrientplot6)
# showing frequency of nutrients by lakes
