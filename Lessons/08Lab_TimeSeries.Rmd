...
# always use inner_join when doing time series to avoid missing data
...
# No NAs
# for assignment, use seq function for all dates. This is check for missing data. Do left join of sequence results with dates, this will put NAs in missing gaps.
...
ggplot(BR_complete, aes(x = Date, y = Rainfall)) +
  geom_line() +
  geom_smooth() +
  labs(x = "Date", y = expression("Rainfall"))
print(rainfall_plot)
...
ggplot(BR_complete, aes(x = Date, y = Temperature_C)) +
  geom_line() + 
  geom_smooth() +
  labs(x = "Date", y = expression("Temperature C"))
print(temp_plot)
...
BR_rainfall_ts <- ts(BR_complete$Rainfall, start = c(1901,1), frequency = 365)
BR_temp_ts <- ts(BR_complete$Temperature_C, start = c(1901,1), frequency = 365)
