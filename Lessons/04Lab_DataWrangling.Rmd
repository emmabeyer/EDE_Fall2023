...
# dplyr filtering. Means grab all the depth that equal 0 in chosen data set
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0)

# Choose multiple conditions to filter
#Can also use unique or levels (since in this case it was a factor) instead of summary
...
> Answer: They are dates and don't need a reference number
...
#Can run sections of the pipe to find error if need
...
ex6 <- NTL.phys.data %>%
  select(lakename,sampledate,depth,temperature_C) %>%
  pivot_wider(
    names_from = depth,
