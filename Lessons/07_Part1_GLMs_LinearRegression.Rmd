...
# this is the prefered way, especailly w/ mutliple
...
>  Significant negative correlation between irradance and depth
...
# this is just dividing the plot screen into 4 sections in 2 by 2 cells
...
```{r, fig.height = 3, fig.width = 4}
# filtering the data set by taking out 0s and outliers
...
# plots are much better now that outliers are gone
par(mfrow = c(2,2), mar=c(4,4,4,4))
...
#see that this is not a good example data set
```
...
# shows the upper plot, to highlight correlation
corrplot.mixed(PeterPaulCorr, upper = "ellipse")
# good for finding more variables to include in your regression
```
...
# use this to find the best fit
step(TPAIC)
TPmodel <- lm(data = Paul.naomit, tp_ug ~ dissolvedOxygen + temperature_C + tn_ug)
summary(TPmodel)
# in this case the adjustments didn't work. It's just the data set, process is correct
```