...
aes is the aestetics or how we want the graph set up.
!!! Ask about why graph is below the chunk and not in the plots tab.
...
```{r, fig.height = 3, fig.width = 4}
#shows the distribution of discharge.mean. Default is set in metadata (here it's 30)
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean))

#increases the binwidth to include more data. Here set to 10
#!!! Ask what the difference is between binwidth and bins
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

#try to use between 5-20 bins
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

#setting what data you want to show by setting the scale of your histogram
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
#showing the histogram of gage height mean
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#example of a line plot. First line is the canvas, subsequent lines are adding data/aestetics to the plot
ggplot(USGS.flow.data) +
...
#line 101 - x=gage.height.mean means look at this value here, and color= means color by mean.approvals. This gives the two categories in different colors. Theme inserts a legend.
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#creates a box plot with defined x and y axis
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

#cut_width shows the distribution of the data in different slices. Closer to a histogram.
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

#combines distribution and range of values in the data. Width of box shows how many values are in that range
ggplot(USGS.flow.data) +
