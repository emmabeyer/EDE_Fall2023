...
> Add notes here: Remember that you're first step is to load your nessasary packages. 
...
str(NTL.phys.data) 
#noticed that sampledate is a factor, need to change to date (see line 38-40)
dim(NTL.phys.data)
...
```{r, results = "hide"}
vignette("dplyr")
#help page on this package
#Pay close attention Single table verbs section
```
...
`&` - collect values if they are all true
...
#filtering row that have 0 in column depth

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0)
#first argument is data set, second argument is whatever I'm trying to filter
NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25)
#first data set, second is depth greater than 0.25
...
#see what lakes have the most observations
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake")
#first provide data set, second incude all data in Paul Lake and Peter Lake
...
#example of how to remove other lakes
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))
#cleaner way of filtering which rows that we want
...
#arranging column depth in ascending order
NTL.phys.data.depth.descending <- arrange(NTL.phys.data, desc(depth))
#arranging column depth in decending order
...
#original data, lakename column, and columns be/w sampledate and temperature_C
...
#adds a new column
#adding new column of temp in F
...
#use this code for intervals that are not numeric, but this is much better to use than the first code
```
...
#first filter by lakename, second select the columns that we want, third create a new column for temp F
#only need to provide our conditions
#this generates everything we did above in 5 lines of code. A cleaner way to sort the date
```
...
#remember to save new filtered data sets as it's own csv file
```
...
We want things to be as clean and reproducible as possible.
