...
#Read packages
library(tidyverse);library(lubridate); library(here) #can run all in one line or separate
...
#show 4 5 3, so data was only collected on Tues, Weds, Thurs
unique(sites$dow)

#Summary - is it meaningful? No be/c its a date and doesn't make sense
summary(sites$dow)
...
#Summary - is it meaningful? Shows as categories. Get counts, which makes more sense
summary(sites$dow)
...
#Not intuitive, will want to have more levels
...
#Plot again
#Last code says to include categories that don't have data
ggplot(sites,aes(x=dow)) +
  geom_bar() + 
...
#View the plot
#Recodes the data, can see in environment
ggplot(sites,aes(x=dow)) +
  geom_bar() + 
  scale_x_discrete(drop=FALSE)
```
