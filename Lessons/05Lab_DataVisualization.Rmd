...
# paste puts everything together, that's why there are spaces w/in the ""
# 'R^{2}' is the equation 'R to the power of 2'
...
ggplot(PeterPaul.chem.nutrients,
      ...
      geom_bar() +
      scale_x_discrete(name="month",drop=FALSE)
# enter break after each comma to clean up code
...
surface_temp_1999_plot + 
  ...
  labs(color='Lake') + 
  theme_wsj()
# change the name of the legend to Lake using labs(color=)
```
...
ggplot(filter(PeterPaul.chem.nutrients),
       aes(y=temperature_C,
           x=sampledate,
           color=depth)) +
  geom_point(size=0.5) +
...
# -1 in direction flips the colors in the sacle legend
