...
O3plot3 <- ggplot(EPAair) +
  ...
  theme_light()
print(O3plot3)
# dark items but still light background

O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic()
print(O3plot4)
# no grid lines, close to the default
```
...
# assigning a theme. 'base_size' increasing font size of labels. Changing axis text to black. Positioning legend (default is on the right)
...
O3plot5 <- ggplot(EPAair) +
  ...
  mytheme
print(O3plot5)  
# specifying theme for plot using defined theme above
...
# alpha changes the transparency and size
# geom_text adds text to plot. 'hjust =' horizonal justification, 1 means left aligning, fontface adds type of text (bold)
# this defines different levels on graph to clarify data
# limits specify ranges of the data
# ylab creates label on the y axis
...
display.brewer.all(n = 9) # names of palettes you can use
...
# rel_heights sizes plot on the same grid
